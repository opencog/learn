...
"
	(define (incr-one atom)
		...
		(if (not (do-have-tv? atom))
			(begin
				(lock-mutex *-count-upgrade-mutex-*)
				(if (not (do-have-tv? atom))
					(fetch-atom atom))
				(do-inc-count! atom CNT)
